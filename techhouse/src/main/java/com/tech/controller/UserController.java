...
public class UserController {
	...
	public String joinPOST(Model model, RedirectAttributes rttr, UserVO vo, @RequestParam(value = "file", required=false) MultipartFile file,
		...
		model.addAttribute("inmember", vo);
		rttr.addAttribute("memberEmail", vo.getEmail());
		rttr.addAttribute("memberId", vo.getId());
	...
	public String EmailConfirm(UserVO vo, Model model) throws Exception{
		...
		model.addAttribute("memberEmail", vo);
	...
	public String loginPOST(HttpServletRequest request, UserVO vo, RedirectAttributes rttr) throws Exception{
		...
		if(vo2 != null) {
			...
			if(userService.emailAuthFail(vo.getId()) != 1) {
	...
	public String findMyID(UserVO vo, Model model, RedirectAttributes rttr) throws Exception {
		
		System.out.println("컨트롤러 출럭 : " + vo);
		
		if(userService.findid(vo) != 1) {
	        ...
	        String rawPw = new TempKey().getKey(5,false); //랜덤키 길이 설정
	        ...
	        sendMail.setSubject("[TechHouse 계정 찾기 입니다.]"); //메일제목
	        sendMail.setText(
	                "<h1>TechHouse</h1>" +
	                "<br>TechHouse에 오신것을 환영합니다!" +
	                "<br>아래 임시 비밀번호로 로그인을 하여 비밀번호를 바꿔주세요." +
	                "<br>임시 비밀번호 : " + rawPw);
	        sendMail.setFrom("sko03kim477@gmail.com", "테크하우스");
	        sendMail.setTo(vo.getEmail());
	        sendMail.send();
	        userService.chgpw(vo);
